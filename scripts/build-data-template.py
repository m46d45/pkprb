...
def extract_events(path: Path) -> list[dict]:
    """Parse HistEvent[] from events.ts (unquoted TS keys → JSON)."""
    text = path.read_text(encoding="utf-8")
    start = text.index("= [") + 2
    end = text.rindex("]") + 1
    blob = text[start:end]
    blob = re.sub(r"(?<=\{|\s)(\w+)\s*:", r'"\1":', blob)
    blob = re.sub(r",\s*]", "]", blob)
    return json.loads(blob)
...
def sheet_petunjuk(wb: Workbook, n_prodi: int, n_pt: int, n_center: int, n_prov: int, n_events: int = 0):
    ws = wb.active
...
def sheet_kode(wb: Workbook, provinces: list[str]) -> None:
    ws = wb.create_sheet("08_Daftar_kode")
...
def ref(col: str, n: int) -> str:
    return f"'08_Daftar_kode'!${col}$2:${col}${n}"
...
def sheet_centers(wb: Workbook, centers: list[dict], provinces: list[str]):
    ...
    for i, h in enumerate(headers, 1):
        ws.cell(1, i, h)
    style_header(ws, len(headers))
    comment(ws, 6, "Kode bahaya dipisah koma, tanpa spasi berlebih. Contoh: gempa,tsunami,longsor. Daftar di 08_Daftar_kode kolom bahaya.")
...
def sheet_historis(wb: Workbook, events: list[dict], provinces: list[str]):
    ws = wb.create_sheet("06_Historis_bencana")
    headers = [
        "id",
        "status",
        "tahun",
    ...
    for i, h in enumerate(headers, 1):
        ws.cell(1, i, h)
    style_header(ws, len(headers))
    comment(ws, 5, "Nama provinsi wajib dari dropdown (38). Satu peristiwa multi-provinsi = beberapa baris.")
    comment(ws, 6, "Kode bahaya dipisah koma. Contoh: gempa,tsunami,likuefaksi. Lihat 08_Daftar_kode.")
    comment(ws, 7, "Korban jiwa absolut (bukan per kapita). Angka resmi BNPB / EM-DAT jika tersedia.")
    r = 2
    for e in events:
        write_row(
            ws,
        ...
        r += 1
    for _ in range(HIST_BLANK):
        write_row(ws, r, ["", "baru"] + [""] * 9, fill_new)
        r += 1
    last = r - 1
    widths(ws, [8, 12, 10, 48, 28, 36, 14, 36, 28, 16, 14])
    nprov = 1 + len(provinces)
    dv(ws, ref("A", 3), f"B2:B{last}")
    dv(ws, ref("J", nprov), f"E2:E{last}")
    ...
    note = ws.cell(
        last + 2,
        1,
        "Skala peta Historis = 1–10 dari ln(1 + total korban absolut per provinsi × bahaya). Karhutla sering understated (hanya korban langsung).",
    )
    note.font = Font(name="Calibri", italic=True, size=10, color=HINT)
...
def build():
    ...
    events = extract_events(ROOT / "src/data/events.ts")
    prov_names = [p["name"] for p in provinces]

    wb = Workbook()
    sheet_petunjuk(wb, len(programs), len(unis), len(centers), len(provinces), len(events))
    ...
    sheet_historis(wb, events, prov_names)
    ...
    print("prodi", len(programs), "pt", len(unis), "centers", len(centers), "prov", len(provinces), "events", len(events))
